...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const geocoders = new MapboxGeocoder({
    accessToken: mapboxgl.accessToken,
    mapboxgl: mapboxgl,
  });
  
  map.addControl(geocoders, 'top-left');
  ...
  const polylineInfo = [
    ...
    {url: "http://localhost/Karipas/src/RoutesPoly/alangilan.json",color: "#ff0000", width: 7, name: "Alangilan", pin:"http://localhost/Karipas/elements/images/alangilanpin.png"},
    {url: "http://localhost/Karipas/src/RoutesPoly/balagtas.json",color: "#00ff00",width: 3, name: "Balagtas", pin:"http://localhost/Karipas/elements/images/balagtas.pin.png"},
    {url: "http://localhost/Karipas/src/RoutesPoly/sanisidro.json",color: "#00ff00",width: 2, name: "San Isidro", pin:"http://localhost/Karipas/elements/images/balagtas.pin.png"},
  ...
  if (latitude !== 0 && longitude !== 0 && targetName != "") {
    console.error(targetName,longitude, latitude);
  setDestinations(targetName, latitude, longitude);
} else {
  console.log("Skipping setDestinations: Latitude or longitude is 0.");
}

  function setDestinations(name,  longitude,latitude) {
    document.getElementById(`popAddress-end`).style.display = "inline-block";
    document.getElementById(`addressText-end`).textContent = name;
    const destinationLngLat = [longitude, latitude];
    // Remove any existing destination marker
    if (endMarker) {
      endMarker.remove();
    }
    endMarker = new mapboxgl.Marker({
      element: createCustomMarker("http://localhost/Karipas/elements/images/mapbox-marker-icon-20px-red.png"),
    }).setLngLat(destinationLngLat).addTo(map);
  }
  ...
  document.querySelectorAll('.tourist-spot-btn').forEach(function(button) {
    button.addEventListener('click', function() {
      const name = button.getAttribute('data-name');
      const latitude = parseFloat(button.getAttribute('data-latitude'));
      const longtitude = parseFloat(button.getAttribute('data-longitude'));
      console.log(name, latitude, longtitude);
      setDestinations(name, latitude, longtitude);
    });
  });
  ...
  async function findClosestPointOnRoutes(coordinates,data) { // find the closest point on each route. Returns an array of sorted closest points 
    try {
      ...
      if (closestPointsArray.length === 0) {
        ...
        document.getElementById('noJeepFound').style.display = "inline-block";
        document.getElementById('jeepsAvailable').style.display = "none";
      ...
      const topClosestPoints = closestPointsArray.slice(0, data ); 
      return topClosestPoints;
    } catch (error) {
  ...
  async function compareAndDrawtheLines(origin, destination) {
    try {
      ...
      if (commonRoutes1.length > 0) {
        ...
        document.getElementById('noJeepFound').style.display = "none";
        ...
        for (const stopPointArray of stopPoints) {
          ...
          if (commonRoutes2.length > 0) {
            ...
            console.log("Stop point does not have common routess.");
          }
